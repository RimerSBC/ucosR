...
/* SD Busy semaphore initialized in fat.c */
extern SemaphoreHandle_t xSDBusy;
/*-----------------------------------------------------------------------*/
/* Get Drive Status                                                      */
/*-----------------------------------------------------------------------*/
...
)
{
   ...
   while (xSDBusy == NULL)
      taskYIELD();
   if (!xSemaphoreTake(xSDBusy, 10))
      return RES_ERROR;
   if (sd_mmc_get_type(SYS_SD_SLOT) & (CARD_TYPE_SD | CARD_TYPE_MMC))
   {
      if (sd_mmc_init_read_blocks(SYS_SD_SLOT, sector, count) == SD_MMC_OK)
         if (sd_mmc_start_read_blocks(buff, count) == SD_MMC_OK)
            if (sd_mmc_wait_end_of_read_blocks(false) == SD_MMC_OK)
            {
               result = RES_OK;
               // This ensures the CPU sees the data the DMA just wrote.
               CMCC->MAINT0.bit.INVALL = 1;
            }
   }
   else
      result = RES_ERROR;
   xSemaphoreGive(xSDBusy);
   return result;
}
...
)
{
   ...
   if (sd_mmc_get_type(SYS_SD_SLOT) & (CARD_TYPE_SD | CARD_TYPE_MMC))
   {
      // INVALL on SAMD51 typically clears and invalidates the entire cache.
      CMCC->MAINT0.bit.INVALL = 1;
      __DSB(); // Data Synchronization Barrier to ensure RAM is updated
